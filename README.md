...
БД состоит из таблицы с строками:
1. `id` - id задачи (Первичный ключ, уникальное значение, автозаполнение, int).
2. `tasks` - Название задачи (Длина значение максимум 255, varchar).
3. `description` - Описание задачи (text).
4. `status` - Статус задачи (Стандартное значение: 0. 0 - В процессе, 1 - Выполнена. Boolean: 0, 1, false, true). 
